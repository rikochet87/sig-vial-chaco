...
function buildChartHtml(type: ChartType): string {
  ...
  const datasets = isPie
    ? `[{ data: ${pcts}, backgroundColor: ${colors}, borderColor: '#1a1a1a', borderWidth: 2, hoverOffset: 12 }]`
    : isStack
    ? `[
    ...
    : `[{ label: 'Red Vial (km)', data: ${totals}, backgroundColor: ${colors}, borderColor: '#1a1a1a', borderWidth: 1, borderRadius: 6 }]`;
  ...
  const options = isPie ? `{
    ...
    plugins: {
      legend: { position: 'bottom', labels: { color: '#FFFFFF', font: { size: 12, weight: 'bold' }, padding: 14, boxWidth: 14 } },
    ...
    scales: {
      ...
      y: { ticks: { color: '#FFFFFF', font: { size: 12, weight: 'bold' } }, grid: { display: false } }
    }
  }`;
...
html,body{width:100%;height:100%;background:#1a1a1a;display:flex;align-items:center;justify-content:center}
    ...
    <View style={styles.container}>
      ...
      <ScrollView style={styles.table} contentContainerStyle={styles.tableContent}>
        <View style={styles.tableHead}>
          <Text style={[styles.thCell, { flex: 2 }]}>Zona</Text>
...
const styles = StyleSheet.create({
  ...
  selectorBtnActive: { backgroundColor: Colors.accent, borderColor: Colors.accentDark },
  selectorLabel: { fontSize: 12, fontWeight: '700', color: Colors.textSecondary },
  selectorLabelActive: { color: Colors.primary },
  chartWrap: { height: 300, backgroundColor: '#1a1a1a' },
  chart: { flex: 1, backgroundColor: '#1a1a1a' },
  ...
  thCell: {
    ...
    color: Colors.accent, textTransform: 'uppercase', letterSpacing: 0.6, textAlign: 'right',
  },
  tableRow: {
    flexDirection: 'row', alignItems: 'center',
  ...
  tableTotal: { backgroundColor: Colors.background, borderTopWidth: 2, borderTopColor: Colors.accent },
  tdZona: { flexDirection: 'row', alignItems: 'center', gap: 8 },
  zonaDot: { width: 10, height: 10, borderRadius: 5 },
  tdText: { fontSize: 13, fontWeight: '600', color: Colors.textPrimary },
  tdCell: { flex: 1, fontSize: 12, color: Colors.textSecondary, textAlign: 'right' },
  tdPct: { color: Colors.accent, fontWeight: '800' },
  tdCenter: { textAlign: 'center' },
  tdBold: { fontWeight: '900', color: Colors.textPrimary },
});
